...
function (_R$Component) {
  _inherits(Simplecomponent, _R$Component);
  ...
  function Simplecomponent(props) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Simplecomponent).call(this, props)); // A private binded function that assign input text value at h1.

    var copyTxt = function (e) {
      _this.scom.textContent(e.target.value, 'simple-h1');
    }.bind(_assertThisInitialized(_this)); // Declaring a new Component and assigning copTxt at it's keyup event.


    _this.scom = _this.Component("#simplecomponent.Simplecomponent\n\t\t\t\th1#simple-h1 Simple Component\n\t\t\t\tinput#simple-txt(placeholder=\"Write something..\", keyup=\"cb\")", copyTxt); // Rendering and inserting Component on a DOM container HTMLElement.
    // this.scom.render()
    // Getting it's h1 child and it's textContent at input text value.

    _this.scom.getChild('simple-txt').value = _this.scom.getChild('simple-h1').textContent;
    return _this;
  }
...
/*#__PURE__*/
function (_R$Component) {
  _inherits(SimpleTodo, _R$Component);

  function SimpleTodo(props) {
    var _this;

    _classCallCheck(this, SimpleTodo);

    // Declaring SimpleTodo's state.
    (_this = _possibleConstructorReturn(this, _getPrototypeOf(SimpleTodo).call(this, props))).state = {
      tasks: 0 // Binding it's own method 'task'.

    };
    _this.task = _this.task.bind(_assertThisInitialized(_this)); // Creating a new Component and rendering it in DOM.

    _this.todo = _this.Component(".SimpleTodo\n\t\t\th1#countertask Lista de Tareas\n\t\t\tinput#todo(keypress=\"task\", placeholder=\"Escribe una tarea\")\n\t\t\tbutton#todobutton(click=\"task\", title=\"Insertar tarea\") Insertar", null, _assertThisInitialized(_this));
    return _this;
  }

  _createClass(SimpleTodo, [{
    key: "task",
    value: function task(e) {
      if ((e.keyCode === 13 || e.type === 'click') && !this.r.emptyString(this.todo.getChild('todo').value)) {
        var newtask = this.Tag(".Task#task-".concat(this.state.tasks, " Tarea ").concat(this.state.tasks, ": ").concat(this.todo.getChild('todo').value));
...
function (_R$Component2) {
  _inherits(SimpleToDo, _R$Component2);
  ...
  function SimpleToDo(props) {
    ...
    (_this2 = _possibleConstructorReturn(this, _getPrototypeOf(SimpleToDo).call(this, props))).state = {
      tasks: 0
    };
    _this2.task = _this2.task.bind(_assertThisInitialized(_this2));
    _this2.todo = _this2.Component(".SimpleTodo\n\t\t\th1#countertask Lista de Tareas\n\t\t\tinput#simptodo(keypress=\"task\", placeholder=\"Escribe una tarea\")\n\t\t\tbutton#simptodobutton(click=\"task\", title=\"Insertar tarea\") Insertar", null, _assertThisInitialized(_this2));
    return _this2;
  }
  ...
  _createClass(SimpleToDo, [{
    ...
    value: function task(e) {
      if ((e.keyCode === 13 || e.type === 'click') && !this.r.emptyString(this.todo.getChild('simptodo').value)) {
        var newtask = this.Tag(".Task#task-".concat(this.state.tasks, " Tarea ").concat(this.state.tasks, ": ").concat(this.todo.getChild('simptodo').value));
        this.setState(function (prevState) {
          return {
            tasks: prevState.tasks + 1
          };
...
/*#__PURE__*/
function (_R$Component) {
  _inherits(Countdown, _R$Component);
  ...
  function Countdown(props) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Countdown).call(this, props)); // Setting Countdown state.

    _this.state = {
      's': 6,
      'jungle': 'Welcome to the jungle Rocfor!' // Binding Countdown's runCounter method.

    };
    _this.runCounter = _this.runCounter.bind(_assertThisInitialized(_this)); // Creating a new Component and assigning it's runCounter to button 'again' click.

    _this.counter = _this.Component("#countdown.Countdown\n\t\t\th1#counter.Counter\n\t\t\tbutton#again.Again(click=\"runCounter\", disabled=\"true\") Replay", null, _assertThisInitialized(_this)); // .render()
  ...
  _createClass(Countdown, [{
    ...
    value: function runCounter() {
      ...
      var btn = this.counter.getChild('again');
      ...
      var counterInterval = null,
        ...
        _this2.counter.innerHTML(_this2.state.s ? _this2.state.s : _this2.state.jungle, 'counter');

        if (!_this2.state.s) {
          _this2.resetState().counter.getChild('again').disabled = false;
...
var Contents = function Contents() {
  ...
  this.content = {
    'main': "class(Container-roc-banner.roc-fluid.co-1.p-2.m-bottom-5)? \nclass(center)## Rocfor.js\nclass(center.thm-subtitle.m-bottom-5)Una sencilla librer\xEDa para construir objetos y componentes reactivos en JavaScript&TRADE;\nclass(Intro-roc.one.M-one-g-70.mfullleft.mfullright.m-bottom-5)? \n### Con Rocfor.js se puede:\nclass(Padded-list.m-bottom-5)* Agilizar tareas mediante m\xE9todos como _unique_ del M\xE9todo Resources, que obtiene un arreglo con valores \xFAnicos, \n* Elaborar r\xE1pida y facilmente simples Elementos HTML *(Tags)*, Bloques HTML *(Blocks)* y Componentes Reactivos *(Components)* para su Proyecto Web,\n* Intercambiar datos con el Servidor mediante su M\xE9todo Ajax,\n* Crear contenidos aplicando [Markdown](https://daringfireball.net/projects/markdown/ \"John Gruber webpage\").\n* Y hasta es posible implementar, mediante su M\xE9todo SPA: ___\xA1Aplicaciones de una sola p\xE1gina!.___  \n?  \nclass(thm-important.center)El deseo es que la Librer\xEDa sea de mucha utilidad.\n?  \nclass(Examples)? \nid(simple-comp)class(Example.M-Example.roc.M-inline.M-top.m-bottom-5)? \nclass(Example-title.p-1.bg-0)#### Un Componente sencillo:\nclass(Example-content.one.M-one-60)? \nclass(Code.Codetext.h-100)+++ javascript\n\t// \xA1Un Componente simple con Rocfor.js!\n\t// A simple Component with Rocfor.js!\n\tclass Simplecomponent extends R.Component {\n\t\tconstructor(props) {\n\t\t\tsuper(props)\n\t\t\t// Una funci\xF3n privada y enlazada a la clase que asigna el valor del input text al h1.\n\t\t\t// A private binded function that assign input text value at h1.\n\t\t\tlet copyTxt = (e => { this.scom.textContent(e.target.value, 'simple-h1') }).bind(this)\n\t\t\t// Creando un nuevo Componente y asignanto la funci\xF3n copyTxt a su evento keyup.\n\t\t\t// Creating a new Component and assigning copyTxt at it's keyup event.\n\t\t\tthis.scom = this.Component(\n\t\t\t\xB4#simplecomponent.Simplecomponent\n\t\t\t\th1#simple-h1 Simple Component\n\t\t\t\tinput#simple-txt(placeholder=\"Write something..\", autofocus=\"true\", keyup=\"cb\")\xB4, copyTxt)\n\t\t\t// Procesando e insertando el Componente en un contenedor HTMLElement del DOM.\n\t\t\t// Rendering and inserting Component on a DOM container HTMLElement.\n\t\t\tthis.scom.render(this.props.container || '')\n\t\t\t// Dando el valor de textContent de h1 al valor del input text.\n\t\t\t// Getting it's h1 child and it's textContent at input text value.\n\t\t\t.getChild('simple-txt').value = this.scom.getChild('simple-h1').textContent\n\t\t}\n\t}\n+++\n?  \nclass(Coderesult.one.M-one-40.thm-oy.p-1)? \nclass(p-top-2.p-bottom-2.p-left-2)#### Resultado:\nid(simplecomp)class(Result)? \n?  \n?  \n?  \n\nid(simple-todo)class(Example.M-Example.roc.M-inline.M-top.m-bottom-5)? \nclass(Example-title.p-1.bg-0)#### Una Lista de Tareas:\nclass(Example-content.one.M-one-60.h-50)? \nclass(Code.Codetext.h-100)+++ javascript\n\t// \xA1Una Lista de tareas con Rocfor.js!\n\t// A Todo List with Rocfor.js!\n\tclass SimpleTodo extends R.Component {\n\t\tconstructor (props) {\n\t\t\t// Declarando el estado interno de la lista de tareas.\n\t\t\t// Declaring SimpleTodo's state.\n\t\t\tsuper(props).state = {tasks: 0}\n\t\t\t// Enlazando a la clase su propio m\xE9todo 'task'.\n\t\t\t// Binding it's own method 'task'.\n\t\t\tthis.task = this.task.bind(this)\n\t\t\t// Creando un nuevo Componente, proces\xE1ndolo e insert\xE1ndolo en el DOM.\n\t\t\t// Creating a new Component and rendering it in DOM.\n\t\t\tthis.todo = this.Component(\n\t\t\t\xB4.SimpleTodo\n\t\t\t\th1#countertask Lista de Tareas\n\t\t\t\tinput#todo(keypress=\"task\", placeholder=\"Escribe una tarea\", autofocus=\"true\")\n\t\t\t\tbutton#todobutton(click=\"task\", title=\"Insertar tarea\") Insertar\xB4, this.task).render()\n\t\t}\n\t\ttask (e) {\n\t\t\tif ((e.keyCode === 13 || e.type === 'click') && !this.r.emptyString(this.todo.getChild('todo').value)) {\n\t\t\t\tlet newtask = this.Tag(\xB4.Task#task-${this.state.tasks} Tarea ${this.state.tasks}: ${this.todo.getChild('todo').value}\xB4)\n\t\t\t\tthis.setState(prevState => ({tasks: prevState.tasks + 1}))\n\t\t\t\te.target.parentElement.appendChild(newtask.getTag())\n\t\t\t\tthis.r.focusAndClean(this.todo.getChild('todo'))\n\t\t\t}\n\t\t}\n\t}\n+++\n?  \nclass(Coderesult.one.M-one-40.thm-oy.p-1)? \nclass(p-top-2.p-bottom-2.p-left-2)#### Resultado:\nid(simpletodo)class(Result)? \n?  \n?  \n?  \n\nid(simple-countdown)class(Example.M-Example.roc.M-inline.M-top.m-top25)? \nclass(Example-title.p-1.bg-0)#### Un Contador regresivo:\nclass(Example-content.one.M-one-60.h-50)? \nclass(Code.Codetext.h-100)+++ javascript\n\t// \xA1Contador regresivo con Rocfor.js!\n\t// Countdown with Rocfor.js!\n\tclass Countdown extends R.Component {\n\t\tconstructor(props) {\n\t\t\tsuper(props)\n\t\t\t// Estableciendo el estado del Contador.\n\t\t\t// Setting Countdown state.\n\t\t\tthis.state = {'s':6, 'jungle':'Welcome to the jungle Rocfor.js!'}\n\t\t\t// Enlazando el m\xE9todo runCounter a la clase.\n\t\t\t// Binding Countdown's runCounter method.\n\t\t\tthis.runCounter = this.runCounter.bind(this)\n\t\t\t// Creando un nuevo Componente y asignando el m\xE9todo runCounter al evento click del bot\xF3n.\n\t\t\t// Creating a new Component and assigning it's runCounter to button 'again' click.\n\t\t\tthis.counter = this.Component(\n\t\t\t\xB4#countdown.Countdown\n\t\t\t\th1#counter.Counter\n\t\t\t\tbutton#again.Again(click=\"runCounter\", disabled=\"true\") Replay\xB4, null, this)\n\t\t\t.render()\n\t\t\t// Inicializando el Contador.\n\t\t\t// Initializing the counter.\n\t\t\tthis.runCounter()\n\t\t}\n\t\trunCounter () {\n\t\t\tlet btn = this.counter.getChild('again');\n\t\t\tif (!btn.disabled) btn.disabled = true\n\t\t\tlet counterInterval = null,\n\t\t\t\t\ttictac = (() => {\n\t\t\t\t\t\tthis.setState(prevState => ({s: prevState.s - 1}))\n\t\t\t\t\t\tthis.counter.innerHTML(this.state.s ? this.state.s : this.state.jungle, 'counter')\n\t\t\t\t\t\tif(!this.state.s) {\n\t\t\t\t\t\t\tthis.resetState().counter.getChild('again').disabled = false\n\t\t\t\t\t\t\tclearInterval(counterInterval)\n\t\t\t\t\t\t}\n\t\t\t\t\t}).bind(this)\n\t\t\tcounterInterval = setInterval(() => tictac(), 1000)\n\t\t}\n\t}\n+++\n?  \nclass(Coderesult.one.M-one-40.thm-oy.p-1.h-100)? \nclass(p-top-2.p-bottom-2.p-left-2)#### Resultado:\nid(simplecountdown)class(Result)? \n?  \n?  \n?  \n?  ",
    'footer': "class(roc.M-inline.middle.download-zone.center.p-1)? \nclass(one.M-one-g-33.M-left.center)? \nclass(thm-title)Descargar:\n[Rocfor.js](https://github.com/Roccouu/rocfor/blob/master/project-dist/rocforjs/rocfor-1.0.0.min.js \"Descargar Rocfor.js\")\n?  \nclass(one.M-one-g-33)? \nclass(roc.center.copycom)? \n[Roccou](https://twitter.com/&#95;roccou \"Roccou\")\nPotos\xED - Bolivia - ".concat(this.date.getFullYear(), "\n?  \n?  \nclass(one.M-one-g-33.M-right)? \nLicencia:\n[MIT](https://github.com/roccouu/rocfor/blob/master/LICENSE \"Rocfor License\")\n?  \n?  "),
    'tuto': "\nclass(Tuto-content.roc.M-inline.M-top)? \nclass(one.M-one-g-80)? \n\nid(init)class(Aside.Tuto-section.m-bottom-5)? \n## Iniciar con Rocfor.js.\n-------\n**Rocfor.js** es una librer\xEDa que ayuda a crear Componentes reactivos mediante c\xF3digo JavaScript.\nPara trabajar con ella, es necesario incluirla como Dependencia de Proyecto.\nEl entorno de Rocfor ofrece dos Clases de las cuales es posible instanciar nuevos objetos.\nEl primero contiene propiedades y m\xE9todos para realizar tareas comunes de programaci\xF3n.\nEl segundo contiene propiedades y m\xE9todos para crear Componentes reactivos adem\xE1s de solventar las tareas comunes de programaci\xF3n.\nEn esta secci\xF3n se muestra la informaci\xF3n b\xE1sica para que pueda familiarizarse con La Librer\xEDa.\nAntes de comenzar:\nclass(Download)Puede descargar Rocfor.js desde su repositorio oficial en GitHub: [Rocfor.js](https://github.com/Roccouu/rocfor \"Rocfor.js\").\nO copiar el CDN en su archivo HTML principal:\nclass(cdn)CDN: `https://cdn.jsdelivr.net/gh/Roccouu/rocfor@master/project-dist/rocforjs/rocfor-1.0.0.min.js`\nO si trabajar con [NPM](https://www.npmjs.com/ \"NPM\") recuerde descargarla como dependencia de proyecto:\nclass(npminstall)Instalaci\xF3n desde NPM: `$ npm install --save rocfor`\n\nEl paso que sigue deber\xE1 ser enlazar Rocfor a un documento HTML, (se puede insertar una etiqueta contendora en `body` -el cuerpo del documento- o simplemente dejar la etiqueta `body` con s\xF3lo la etiqueta script mediante la cual se llama a *La Librer\xEDa*:\n\n### Llamar a La Librer\xEDa desde su plantilla HTML:\nclass(Code)+++ html\n\t<!DOCTYPE html>\n\t<html lang=\"es\">\n\t<head>\n\t\t<meta charset=\"UTF-8\">\n\t\t<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n\t\t<link rel=\"stylesheet\" href=\"./css/rocfor-1.0.0.min.css\">\n\t\t<title>Rocfor.js</title>\n\t</head>\n\t<body>\n\t\t<div id=\"container\"></div> <!-- opcional -->\n\n\t\t<!-- Enlazando Rocfor.js en el proyecto -->\n\t\t<script src=\"./rocfor-1.0.0.min.js\"></script>\n\t</body>\n\t</html>\n+++\n\nAhora todo el entorno Rocfor.js est\xE1 disponible en el proyecto ya que fue registrado en El Objeto **window** de JavaScript.\nLa forma de acceder al entorno Rocfor.js es mediante la letra: `R`.\nEl algoritmo de inicializaci\xF3n de Rocfor.js verifica que ninguna otra librer\xEDa o *framework* JavaScript est\xE9 registrado en el objeto window con el nombre *R*, pero si as\xED fuere, se ejecutan inmediatemente otras alternativas:\nclass(Padded-list.m-bottom-5)\t1.\tRocfor.js dispondr\xE1 el t\xE9rmino ***Rocfor*** como el nombre de su entorno. \n\t1.\tPero si \xE9ste \xFAltimo tambi\xE9n ya fue registrado, se dispondr\xE1 el t\xE9rmino ***Rocforjs*** como el nombre definitivo.\n\t1.\tSi existiera la casualidad de que los tres nombres anteriores ya residieran en El Objeto window, Rocfor.js sencillamente cancelar\xE1 su registro denegando as\xED, el uso de cualquiera de sus m\xE9todos.  \nRocfor.js muestra un aviso en la consola del navegador (Ejm.: R ready!) el cual menciona el nombre que habilit\xF3 para ser usado en el proyecto.\nUna vez exista Rocfor.js en el entorno window, ya puede llamar a __Rocfor.js__ desde cualquier funci\xF3n o clase JavaScript mediante los acr\xF3nimos mencionados, es decir, las clases que cree ya pueden heredar de Rocfor.js y desde una funci\xF3n s\xF3lo deber\xE1 crear una nueva instancia en una variable:\n\n### Llamar a Rocfor.js desde una Clase JavaScript:\nclass(Code)+++ javascript\n\t'use strict'\n\tclass MiClase extends R {\n\t\tconstructor(props) {\n\t\t\tsuper(props)\n\t\t\t// Todas las tareas que desee realizar con las funciones de Rocfor.js.\n\t\t}\n\t\t// Todas las tareas que desee realizar con las funciones de Rocfor.js.\n\t}\n+++\n### Llamar a Rocfor.js desde un IIFE JavaScript:\nclass(Code)+++ javascript\n\t'use strict'\n\t;((function (r){\n\t\t// Todas las tareas que desee realizar con las funciones de Rocfor.js.\n\t})(new R()));\n+++\n\nEl entorno que La Librer\xEDa pone a disposici\xF3n con esta acci\xF3n permite trabajar con los siguientes M\xE9todos y Propiedades:\nclass(Padded-list.m-top5)+ **Ajax** (M\xE9todo): Precisamente para trabajos as\xEDncronos con el servidor, \n+ **version** (Propiedad): Es una propiedad que le proporcionar\xE1 la versi\xF3n de La Librer\xEDa que utiliza,\n+ **r** (o Recursos. Propiedad de tip Objeto): Un conjunto de funciones que permiten realizar tareas b\xE1sicas como: el cambio de clases a un Elemento HTML, tratamiento de Cadenas, tratamiento de Arreglos, de Objetos HTML y Objetos JavaScript,\n* __props__ (Propiedad): Un par\xE1metro que puede adopar el tipo de dato que usted requiera,\n* __Error__: M\xE9todo que permite el manejo de errores en la aplicaci\xF3n,\n* __Tag__: El M\xE9todo N\xFAcleo de Rocfor.js, le permitir\xE1 crear Elementos HTML de manera pr\xE1ctica, divertida y en un \xE1mbito JavaScript nativo,\n+ **Block**: M\xE9todo que le permitir\xE1 crear Bloques de uno o varios Elementos HTML,\n* __SPA__: Mediante este M\xE9todo podr\xE1 crear Aplicaciones de una sola p\xE1gina, y,\n* __Mdown__: Un M\xE9todo inspirado en la idea de [John Gruber](https://daringfireball.net/projects/markdown/ \"John Gruber webpage\") pero que en realidad es un tipo de Markdown pensado para programadores.  \nCuando desee trabajar con Componentes, Rocfor.js le ofrecer\xE1 un entorno pensado espec\xEDficamente para esta tarea y requerir\xE1 invocar al M\xE9todo Component espec\xEDfico de Rocfor.js.\nLos m\xE9todos que se explican en este tutorial son ___Tag___, ___Block___ y el M\xE9todo ___Component___ de la Clase R.Component (La segunda Clase que conforma Rocfor.js. Ser\xE1 explicada m\xE1s adelante)\n?  \n\nid(tag)class(Aside.Tuto-section.m-bottom-5)? \n\n## Crear un Elemento HTML mediante el M\xE9todo Tag.\n-----------\nTras tener la base, se pueden crear Elementos HTML t\xE1n simples como complejos los desee:\n\nclass(Code)+++ javascript\n\t'use strict'\n\tclass MiClase extends R {\n\t\tconstructor(props) {\n\t\t\tsuper(props)\n\t\t\tthis.mytag = this.Tag('\xA1Hola Mundo!')\n\t\t\tconsole.log(mytag) // mytag es ahora una instancia de Tag.\n\t\t}\n\t}\n+++\n\nCon esa simple ejecuci\xF3n, la variable `mytag` ahora, posee un objeto que ofrece algunos m\xE9todos y una propiedad, visibles en la consola del navegador:\n\nclass(Code)+++ javascript\n\tTag\n\t\tdelTag: f()\n\t\tgetTag: f()\n\t\tsetTag: f(where)\n\t\ttagobject: {tag: div, idlist: null...}\n\t\ttagstrings: {tagstring: 'div \xA1Hola Mundo!'...}\n\t\t...\n+++\n\nSi a continuaci\xF3n escribe `mytag.setTag()`, la ventana del navegador le mostrar\xE1 \xA1Hola Mundo!.\nEntonces, \xBFqu\xE9 piensa que ocurrir\xEDa si ejecuta el siguiente c\xF3digo?:\nclass(Code)+++ javascript\n\t'use strict'\n\tclass MiClase extends R {\n\t\tconstructor(props) {\n\t\t\tsuper(props)\n\t\t\tthis.mytag-a = this.Tag('\xA1Hola Mundo!')\n\t\t\tthis.mytag-b = this.Tag('\xA1Hola Mundo! |[\xA1Saludos desde Potos\xED!]')\n\t\t\tthis.mytag-c = this.Tag('h1 \xA1Hola Mundo! |[em Tutorial Rocfor.js.]')\n\t\t\tthis.mytag-a.setTag();\n\t\t\tthis.mytag-b.setTag('contanier');\n\t\t\tthis.mytag-c.setTag({'after':'container'});\n\t\t}\n\t}\n+++\n\nEfectivamente, apreciar\xE1, todos los contenidos de cada objeto, pero ese es s\xF3lo el resultado de todo un proceso que le pone en sus manos: ***El poder de trabajar de manera directa con Objetos Elementos HTML desde JavaScript***.\nEntonces, si mira la estructura del DOM en su documento HTML en tiempo de ejecuci\xF3n, notar\xE1 que _Tag_ cre\xF3 un Elemento `div` para el \xA1Hola Mundo!; y as\xED, un `div` con un nodo hijo `span` para `mytag-b` y un `h1` con un nodo hijo `em` para `mytag-c`, de aqu\xED en m\xE1s, s\xF3lo tendra que invocar al elemento que cada intancia (mytag-a, b o c) posea mediante los m\xE9todos que Tag le ofrece.\nObserve que las cadenas enviadas a Tag son: O cadenas normales, o cadenas con una sintaxis.\nRocfor.js simplific\xF3 la manera de escribir etiquetas HTML para que usted obtenga Elementos HTML concretos; es decir:\nEl algoritmo de Rocfor.js se basa en la sintaxis HTML para escribir etiquetas, que, en general, poseen tres partes: _un nombre (tagName)_, _Atributos_ y un _Contenido_, pero como es conocido, se deben escribir entre los signos mayor y menor que (< ... >):\n\nclass(Code)+++ html\n\t<NombreEtiqueta Atributos> Contenido </NombreEtiqueta>\n\t<NombreEtiqueta>\n\t\tContenido\n\t\t<NombreEtiquetaNodoHijo Atributos>\n\t\t\tContenido nodo hijo\n\t\t</NombreEtiquetaNodoHijo>\n\t</NombreEtiqueta>\n+++\n\nRocfor.js simplifica esta escritura a:\n\nclass(Code)+++ rocjs\n\tNombreEtiqueta#id.Clases(Atributos=\"atributo\") Contenido\n\tNombreEtiqueta Contenido |[NodoHijo#id.Clases(Atributos=\"atributo\") Contenido]\n+++\nDonde:\nclass(Padded-list.m-bottom-5)* **NombreEtiqueta:** Opcional, si se omite, Tag crea por defecto un `div`. \n* **Atributos:** #id, .class \xF3 (id=\"un-id\", class=\"Una-clase\", title=\"Un t\xEDtulo\", ...), Todos los atributos son opcionales. Los atributos tienen dos partes (desde luego, ambas opcionales), la primera es denominada ___atributos iniciales___, en esta parte pueden ser incluidos dos tipos de atributos: _id_ y _class_; los ___id___ pueden ser escritos antecedidos por un caracter numeral (hash #) y las ___clases___ antecedidas por un punto (dot .). Estos atributos deben se deben escribir sin espacios ni caracteres diferentes a una cadena regular, ejemplos: #un-id.Una-clase.otra-clase, .Una-clase#un-id.Otraclase, .Una-clase.otra-clase#un-id. El segundo tipo de atributos son los poco comunes o diferentes a los id y clases, deben ir entre par\xE9ntesis como los atributos normales de las etiquetas HTML normales pero separados por coma y un espacio, ejemplos: (href=\"#\", title=\"Un t\xEDtulo\"), los id y clases tambi\xE9n pueden escribirse como atributos del segundo tipo, ejemplo: (href=\"#\", id=\"un-id\", title=\"Un t\xEDtulo\", class=\"Una-clase otra-clase\"). Al ser opcional el nombre de la etiqueta, el tagString puede comenzar con cualquier atributo de cualquiera de los dos tipos mencionados.\n* **Contenido:** Cualquier contenido incluido Markdown o ninguno. Para poder escribir contenidos en la tagString, debe dejarse un espacio al inicio del contenido, ejemplo: h1 Un contenido, #un-id Un contenido, .Una-clase Otro contenido, (id=\"un-id\") Un contenido. Si env\xEDa s\xF3lo un contenido, el espacio inicial es innecesario, ejemplo: Un contenido.\n* **Nodos hijos:** Entre los s\xEDmbolos |[ y ] Si se omite `nombreEtiqueta` en nodo hijo, Tag crea por defecto un `span`.  Luego,la escritura de los nodos hijo sigue las mismas reglas ya citadas.  \n\nPor tanto el modo \xE1gil de escritura de etiquetas se resume a escribir cadenas bajo ese formato.\nEn Rocfor.js, estas cadenas se denominan **tagString**, porque son cadenas del tipo dato ___JavaScript String___ y representan a una Etiqueta HTML o ___tag___ (de ah\xED su nombre: tagString).\nEntonces, las siguientes Etiquetas HTML:\n\nclass(Code)+++ Rocfor.js\n\t<NombreEtiqueta atributo=\"un atributo\">Contenido</NombreEtiqueta>\n\t<div>Ser\xE9 una etiqueta div</div>\n\t<div class=\"Div\">Ser\xE9 otra etiqueta div con una clase Div</div>\n\t<div id=\"Div\">Ser\xE9 otra etiqueta div con un id Div</div>\n\t<header id=\"header\" class=\"Header\" title=\"Un header\">Contenido de texto.</header>\n\t<nav class=\"Mi-nav\">\n\t\t<li>\n\t\t\t<a href=\"#\">Un enlace</a>\n\t\t</li>\n\t</nav>\n\t<div>Tengo un contenido Markdown</div>\n\t<div>\n\t\tTengo un contenido Markdown\n\t\t<em>yo, tambi\xE9n</em>\n\t</div>\n\t<div>\n\t\tTengo un contenido\n\t\t<strong>yo un contenido Markdown</strong>\n\t</div>\n+++\nPueden escribirse en formato _tagString_ as\xED:\nclass(Code)+++ Rocfor.js\n\tNombreEtiqueta(Atributo=\"un atributo\") Contenido\n\tSer\xE9 una etiqueta div\n\tdiv.Div Ser\xE9 otra etiqueta div con una clase Div\n\tdiv#Div Ser\xE9 otra etiqueta div con un id Div\n\theader#header.Header(title=\"Un header\") Contenido de texto.\n\tnav(class=\"Mi-nav\") |[li |[a(href=\"#\") Un enlace]]\n\tdiv MD: Tengo un contenido Markdown\n\tdiv MD: Tengo un contenido Markdown |[em MD: yo, tambi\xE9n]\n\tdiv Tengo un contenido |[strong MD: yo un contenido Markdown]\n+++\n\n...En base a estos ejemplos, la creaci\xF3n de etiquetas queda a su imaginaci\xF3n.\nLos requisitos son:\nclass(Padded-list.m-bottom-5)\t1. si utiliza los s\xEDmbolos [ y ] siempre escriba el s\xEDmblo de cierre anteponiendo una barra diagonal: /]. \n\t1. Si desea escribir los s\xEDmbolos |[ juntos, deber\xE1 usar | antes de [.\n\t1. Los contenidos de las etiquetas HTML deben estar escritos en una sola l\xEDnea, a menos que el contenido que env\xEDe sea un Markdown. Se recomienda adem\xE1s que escriba contenidos cortos, la idea es que con esta herramienta pueda crear interfaces robustas para su p\xE1gina o aplicaci\xF3n web; el tratamiento de cadenas extensas recaer\xEDa en un uso extremo de JavaScript, un ejemplo de ello es este sitio web que fue creado con un 70% de c\xF3digo JavaScript incluidos los contenidos, funciona, pero es un extremo. Recuerde, la finalidad de Rocfor.js es la creaci\xF3n de Componentes.\n\t1. Si desea escribir Markdown como contenido de un Elemento HTML, la cadena siempre deber\xE1 comenzar por `MD: contenido Markdown`.\n\t1. Si descubre alg\xFAn mal funcionamiento, sin dudarlo, consulte la documentaci\xF3n de Rocfor.js; y si desea colaborar con el mejoramiento del c\xF3digo, es libre de hacerlo.  \n\n__Tag__ adem\xE1s de una _tagString_ puede recibir otros tres par\xE1metros opcionales:\nclass(Padded-list.m-bottom-5)\t1. _fn_: que es una referencia a una funci\xF3n que puede asignar a un evento del Elemento HTML que construye, \n\t1.\t_scope_: que es el \xE1mbito de la funci\xF3n o clase JavaScript en la que trabaja y donde se encuentra la nueva instancia de Tag que haya declarado, este par\xE1metro le ser\xE1 \xFAtil sobretodo cuando trabaja con Objetos o Clases JavaScript en donde tambi\xE9n podr\xE1 crear simplemente un constructor del objeto Tag y asignarlo a una variable o propiedad, este es otro modo de trabajar con Tag. Vea la [Documentaci\xF3n de Rocfor.js](".concat(this.docs, " \"Descargar el libro de Rocfor.js\") para obtener informaci\xF3n m\xE1s detallada; y\n\t1.\t_rvars_: un modelo de variables propuestas por Rocfor.js para cuando desee insertar variables en las tagString. Este par\xE1metro es el acr\xF3nimo de __Rocfor.js variables__ y consiste en un argumento especial que fue pensado para enviar variables a Rocfor.js dentro de las cadenas _blockString_, su formato es: Caso 1: `$variable, Caso 2: $$$variable-en-objeto-javascript \xF3 Caso 3: $(alguna operaci\xF3n aritm\xE9tica)`, __aunque su uso es poco recomendado en el primer y tercer casos__; con la primera forma, podr\xE1 enviar cualquier variable declarada a partir del \xE1mbito inmediatamente superior al actual, por ejemplo, si declar\xF3 una variable `let x = 0;`, para enviar esta variable a un _tagString_ deber\xE1 escribir el nombre de la variable _x_ antecedido por el caracer $, as\xED: `this.Tag('span Soy la variable x en un contenido y mi valor es: $x')`. Desde luego, los valores que retornen estas variables deber\xE1n ser num\xE9ricos o cadenas. La segunda forma de _rvars_ buscar\xE1 el nombre de la variable en un objeto que env\xEDe en el par\xE1metro _rvars_; y la tercera forma le permitir\xE1 evaluar cualquier operaci\xF3n matem\xE1tica en un _blockString_. Consulte la [Documentaci\xF3n de Rocfor.js](").concat(this.docs, " \"Descargar el libro de Rocfor.js\") para obtener informaci\xF3n m\xE1s detallada.  \n\xBFSon\xF3 interesante?, qu\xE9 tal si ahora obtiene adem\xE1s de un solo Elemento HTML, \xA1un Bloque de Elementos HTML!.\n?  \n\nid(block)class(Aside.Tuto-section.m-bottom-5)? \n## Crear un Bloque de Elementos HTML mediante el M\xF3dulo Block:\n-------\nComo habr\xE1 notado, el uso de los M\xF3dulos de Rocfor.js es ampliamente sencillo.\nPuesto que, para crear un Elemento HTML con el M\xF3dulo Tag requiere una sola cadena de una l\xEDnea, obviamente, para crear un Bloque de Elementos HTML se requieren una o m\xE1s l\xEDneas de cadenas _tagString_.\nEl conjunto de cadenas _tagString_ empleadas en Block se denominan __blockString__, entonces, el siguiente __blockString__:\n\nclass(Code)+++ javascript\n\t...\n\tthis.myblock-a = this.Block('\xA1Hola Mundo!')\n\tthis.myblock-b = this.Block(\xB4div#main.Main\n\t\th1 \xA1Hola Mundo!\n\t\th2 Tutorial Rocfor.js.\n\t\tp Contenido de prueba para aprender |[strong Rocfor.js.]\n\t\tp Esto es: |[em \xA1divertido!]\xB4)\n\t...\n+++\nRepresenta a:\nclass(Code)+++ javascript\n\t...\n\t<div>\xA1Hola Mundo!</div>\n\t<div id=\"main\" class=\"Main\">\n\t\t<h1>\xA1Hola Mundo!</h1>\n\t\t<h2>Tutorial Rocfor.js.</h2>\n\t\t<p>Contenido de prueba para aprender <strong>Rocfor.js.</strong></p>\n\t\t<p>Esto es: </em>\xA1divertido!</em></p>\n\t</div>\n\t...\n+++\nComo habr\xE1 notado, para obtener una instancia del objeto Block debe asignar este objeto con su cadena _blockString_ a una variable. Tras realizar este paso, Block le devolver\xE1 un objeto Block:\nclass(Code)+++ javascript\n\tBlock\n\t\tblock: div\n\t\tblockChild: [{...}]\n\t\tblockmain: {...}\n\t\tblockstrings: {blockstring:..., blockString...}\n\t\tclassList: null\n\t\tevents: null\n\t\tidDOM: null\n\t\tidList: null\n\t\t__proto__\n\t\t\tblockstringChild: f(index-or-id-child)\n\t\t\tdelBlock: f()\n\t\t\tgetBlock: f()\n\t\t\tsetBlock: f(where)\n\t\t\tgetChild: f(index-or-id-child)\n\t\t\tinnerHTML: f(htmltxt, index-or-id-child)\n\t\t\ttextContent: f(txt, index-or-id-child)\n+++\nMediante el cual ya puede manejar el nuevo Bloque de Elementos HTML en su proyecto.\nComo Tag, Block tambi\xE9n puede recibir otros par\xE1metros adem\xE1s de su _blockString_, estos son: _fn_, _scope_ y _rvars_.\n?  \n\nid(comp)class(Aside.Tuto-section.m-bottom-5)? \n## Crear un Componente mediante el M\xF3dulo Component:\n-------\nSi comprendi\xF3 la esencia de Rocfor.js mediante los M\xF3dulos Tag y Block, construir Componentes altamente profesionales para las interfaces de sus aplicaciones mediande el M\xE9todo __Component__, ser\xE1 pr\xE1cticamente un pasatiempo.\nEl principio contin\xFAa siendo la escritura de _tagStrings_ que para el caso de los Componentes se denominan ___componentStrings___.\nEste M\xE9todo se utiliza de manera similar a Block, es decir, requiere par\xE1metros similares: _componentString_, _fn_, _scope_ y _rvars_ y tal cual Block, Component le devolver\xE1 un objeto con m\xE9todos y propiedades:\nclass(Code)+++ javascript\n\tComponent\n\t\tcomponent: div\n\t\tcompChild: [{...}]\n\t\tcompmain: {...}\n\t\tcompstrings: {blockstring:..., blockString...}\n\t\tclassList: null\n\t\tevents: null\n\t\tidDOM: null\n\t\tidList: null\n\t\t__proto__\n\t\t\tcompstringChild: f(index-or-id-child)\n\t\t\tdelComponent: f()\n\t\t\tgetComponent: f()\n\t\t\trender: f(where)\n\t\t\tgetChild: f(index-or-id-child)\n\t\t\tinnerHTML: f(htmltxt, index-or-id-child)\n\t\t\ttextContent: f(txt, index-or-id-child)\n+++\nLa Clase r.Component es una especie de sublibrer\xEDa de Rocfor.js, pero es independiente de su \xE1mbito (scope); por esta raz\xF3n, cuando cree una Clase u Objeto JavaScript, en lugar de heredar de R, deber\xE1 heredar de R.Component, as\xED:\nclass(Code)+++ javascript\n\t'use strict'\n\tclass MiClase extends R.Component {\n\t\tconstructor(props) {\n\t\t\tsuper(props)\n\t\t}\n\t}\n+++\nY `MiClase` adoptar\xE1 un \xE1mbito espec\xEDfico para trabajar con R.Component, en el que contar\xE1 con los siguientes m\xE9todos y propiedades:\nclass(Padded-list.m-top5)* __props__ (Propiedad): Una propiedad que adopta el valor que env\xEDe en el par\xE1metro _props_ que recibe ___R.Component___, \n* __r__: Los recursos de Rocfor.js mediante los cuales podr\xE1 realizar tareas comunes,\n* __state__: El estado interno del Componente.\n* __version__: Propiedad que le devuelve, eso: la versi\xF3n de Rocfor.js,\n* __Error__: M\xE9todo que permite el manejo de errores en el Componente,\n* __setState__: M\xE9todo que permite el manejo de los estados del Componente,\n* __resetState__: M\xE9todo que permite la reinicializaci\xF3n de los estados del Componente,\n* __Tag__: M\xE9todo para construir Tags,\n* __Block__: M\xE9todo para construir Blocks,\n* __Component__: M\xE9todo para construir Componentes, y,\n* __Mdown__: M\xE9todo para crear contenidos cortos mediante la sintaxis Markdown, en este caso, para desarrolladores.  \nAhora que conoce lo b\xE1sico de Rocfor.js, \xBFQuiere crear un Componente?...\n?  \n\nid(todo)class(Aside.Tuto-section.m-bottom-5)? \n## Crear una sencilla Lista de Tareas mediante el M\xF3dulo Component:\n-------\nObserve el siguiente c\xF3digo JavaScript:\nclass(Code)+++ javascript\n\t// Heredar de R.Component.\n\tclass SimpleTodo extends R.Component {\n\t\t// Recibir valores en el par\xE1metro props del Componente.\n\t\tconstructor (props) {\n\t\t\tsuper(props)\n\t\t\t// Creando estados para el Componente.\n\t\t\tthis.state = {tasks: 0}\n\t\t\tthis.task = this.task.bind(this)\n\t\t\t// Creando una instancia para un nuevo Componente.\n\t\t\tthis.todo = this.Component(\xB4.SimpleTodo\n\t\t\t\th1#countertask Lista de Tareas\n\t\t\t\tinput#todo(keypress=\"task\", placeholder=\"Escribe una tarea\", autofocus=\"true\")\n\t\t\t\tbutton#todobutton(click=\"task\", title=\"Insertar tarea\") Insertar\xB4, this.task).render(this.props.container || '')\n\t\t}\n\t\ttask (e) {\n\t\t\tif ((e.keyCode === 13 || e.type === 'click') && !this.r.emptyString(this.todo.getChild('todo').value)) {\n\t\t\t\tlet newtask = this.Tag(\xB4.Task#task-${this.state.tasks} Tarea ${this.state.tasks}: ${this.todo.getChild('todo').value}\xB4)\n\t\t\t\tthis.setState(prevState => ({tasks: prevState.tasks + 1}))\n\t\t\t\te.target.parentElement.appendChild(newtask.getTag())\n\t\t\t\tthis.r.focusAndClean(this.todo.getChild('todo'))\n\t\t\t}\n\t\t}\n\t}\n+++\nAhora observe el resultado:\nid(todores)? \nclass(Coderesult.one.M-one-40.thm-oy.p-1)? \nclass(p-top-2.p-bottom-2.p-left-2)#### Resultado:\nid(mysimpletodo)class(Result)?\n?  \n?  \n?  \n\xBFLo comprendi\xF3?\nSi la respuesta es: S\xED.\n\xA1Excelente! Rocfor.js es lo suyo, de otro modo, descuide, a continuaci\xF3n se describe el c\xF3digo l\xEDnea a l\xEDna (Se asume que tiene conocimientos del lenguaje JavaScript, para este caso, la Especificaci\xF3n ES6):\n### Explicaci\xF3n del C\xF3digo fuente:\nCon las l\xEDneas 2. a 5. crea una nueva clase SimpleTodo que extiende del M\xF3dulo Component de Rocfor.js. El par\xE1metro `props` es en este caso un objeto JavaScript: `{container:'container'}` que poseer\xE1 una propiedad container cuyo valor ser\xE1 el id de un Elemento HTML del DOM donde se insertar\xE1 el nuevo Componente Elemento HTML Lista de tareas que se est\xE1 creando:\nclass(Code)+++ javascript\n\tclass SimpleTodo extends R.Component {\n\t\tconstructor (props) {\n\t\t\tsuper(props).state = {tasks: 0}\n+++\n`props` es la abreviaci\xF3n de propiedades (properties) que se facilitan a la nueva instancia de la Clase JavaScript que se cree, este par\xE1metro, por lo general ser\xE1 un objeto JavaScript, pero cuando vea por conveniente podr\xE1 enviar otro tipo de dato. En general, Rocfor.js siempre espera un valor cualquiera en este par\xE1metro y lo env\xEDe u omita, siempre crear\xE1 una propiedad interna `this.props` con `undefined` como valor predeterminado. Por esa raz\xF3n, en este caso se env\xEDa un objeto con la propiedad container para que albergue a la Lista de Tareas.\nA continuaci\xF3n las l\xEDneas 7. y 8.:\nclass(Code)+++ javascript\n\tsuper(props)\n\tthis.state = {tasks: 0}\n\tthis.task = this.task.bind(this)\n+++\nES6 recomienda que despu\xE9s de inicializar el m\xE9todo constructor de la Clase, se llame al constructor de la Clase Padre mediante la sentencia `super`.\nA continuaci\xF3n se usa la propiedad `state` de Rocfor.js a la cual se le asigna un objeto con una propiedad tasks con valor inicial de cero: `{tasks: 0}`. Recuerde, tras heredar de R.Component, la Clase Hija dispondr\xE1 en su \xE1mbito de todos los m\xE9todos y propiedades de la Clase Padre y tanto `state, setState() y resetState()` estar\xE1n siempre disponibles.\nEn la l\xEDnea 8. se enlaza un m\xE9todo para `SimpleTodo` denominado `tasks` al que se enlaza de manera expl\xEDcita el \xE1mbito de `SimpleTodo` mediante el m\xE9todo `bind`.\nEn seguida est\xE1n las l\xEDneas 10. a 13. que en realidad, podr\xEDan contar como una sola l\xEDnea:\nclass(Code)+++ javascript\n\tthis.todo = this.Component(\xB4.SimpleTodo\n\th1#countertask Lista de Tareas\n\tinput#todo(keypress=\"task\", placeholder=\"Escribe una tarea\", autofocus=\"true\")\n\tbutton#todobutton(click=\"task\", title=\"Insertar tarea\") Insertar\xB4, this.task).render(this.props.container || '')\n+++\nLo que hace esta sentencia es asignar a una nueva propiedad de `SimpleTodo` una nueva instancia de Component de Rocfor.js en la cual se env\xEDa un _componentString_ que posee cuatro _tagStrings_.\nclass(Padded-list.m-bottom-5)* Como ver\xE1, el _tagString_ de la l\xEDnea 10. corresponde a una Etiqueta `div` con una clase `SimpleTodo`. \n* En la l\xEDnea 11. se especifica un _tagString_ para una Etiqueta HTML `h1` con id `countertask` y un contenido `Lista de Tareas`.\n* La siguiente l\xEDnea describe un `input` de tipo `text`, aqu\xED debe observar especialmente, el atributo `keypress=\"task\"` del _tagString_, \xBFse pregunta qu\xE9 tipo de instrucci\xF3n o atributo es?, la respuesta es sencilla, se trata del evento `onkeypress` que se declara en la lista de atributos del Elemento HTML que se intenta crear, note tambi\xE9n que se asigna a este evento el m\xE9todo `task` de la Clase que fue declarado en la l\xEDnea 8., \xBFtiene curiosidad por saber c\xF3mo se presenta este evento en el Elemento HTML que corresponde a este _tagString_ una vez inserto en el DOM?, b\xFAsquelo y obs\xE9rvelo, _es una recomendaci\xF3n_, as\xED como tambi\xE9n lo es, el que se dirija a la opci\xF3n Documentaci\xF3n de esta Aplicaci\xF3n Web y descargue el Libro que le entrega todos los detalles de La Librer\xEDa.  \n\nEn la l\xEDnea 13., se tiene el \xFAltimo _tagString_ del Componente, se trata de un bot\xF3n de id `todobutton` que a su vez tiene un par de atributos y un contenido, entre sus atributos, aparece otro evento: `click=\"task\"`, luego, despu\xE9s del _componentString_, se env\xEDa un segundo par\xE1metro, el de _fn_, \xBFlo recuerda?, precisamente, \xE9ste, hace referencia al m\xE9todo declarado en la l\xEDnea 8. y que el Componente `todo` ejecutar\xE1 en los eventos `onkeypress y onclick` de los Elementos HTML `input todo y button todobutton`.\nA continuaci\xF3n de la declaraci\xF3n de Component, aparece con el patr\xF3n de encadenamiento, un m\xE9todo, `render`, precisamente, \xE9ste m\xE9todo corresponde al Objeto Component que el M\xF3dulo Component devuelve tras su instanciaci\xF3n a la propiedad `this.todo`, con esta sentencia, se instruye a JavaScri\u1E55t que una vez creado el Componente HTML para `this.todo` lo inserte en el Elemento HTML residente en el DOM `'container'` que fue enviado en el par\xE1metro `props`, o si \xE9ste tiene un valor nulo, vac\xEDo, falso o falsy, etc., env\xEDe una cadena vac\xEDa; luego, ...s\xED. Rocfor.js se encargar\xE1 de buscar el Elemento HTML `'container'` voluntariosa y afanadamente, en caso de fracasar en su intento, tomar\xE1 la decisi\xF3n de insertar de modo predeterminado al nuevo Componente en el Elemento HTML `body` del DOM.\nAhora, toca a las siguientes y \xFAltimas l\xEDneas 15. a 20., que corresponden al m\xE9todo `task` de la Clase, que en esencia se encarga de crear un nuevo Elemento HTML e insertarlo a continuaci\xF3n del bot\xF3n `todobutton`, como un Elemento Hijo del Elemento HTML Padre del Componente cuya clase es `SimpleTodo`:\nclass(Code)+++ javascript\n\ttask (e) {\n\t\tif ((e.keyCode === 13 || e.type === 'click') && !this.r.emptyString(this.todo.getChild('todo').value)) {\n\t\t\tlet newtask = this.Tag(\xB4.Task#task-${this.state.tasks} Tarea ${this.state.tasks}: ${this.todo.getChild('todo').value}\xB4)\n\t\t\tthis.setState(prevState => ({tasks: prevState.tasks + 1}))\n\t\t\te.target.parentElement.appendChild(newtask.getTag())\n\t\t\tthis.r.focusAndClean(this.todo.getChild('todo'))\n\t\t}\n\t}\n+++\nLa condicional `if` eval\xFAa si el evento que invoca este m\xE9todo fue disparado por la pulsaci\xF3n del bot\xF3n izquierdo del dispositivo rat\xF3n o tras presionar la tecla de c\xF3digo 13 equivalente a `Enter` del teclado, adem\xE1s de asegurarse de que el `input text` del Componente tiene contenido de texto utilizando para este fin el m\xE9todo `emptyString` del M\xF3dulo ___r___ (Resources) de Rocfor.js; Si todas estas condiciones se cumplen, se crea en una nueva variable `newtask` un nuevo Objeto Tag al cual se env\xEDa una _tagString_ din\xE1mica que tendr\xE1 por `class=\"Task\", id=\"task-0, 1, 2...\"` y un contenido que adoptar\xE1 un formato: `Tarea 0, 1, 2, ...: El valor que escriba en el Elemento HTML input text`.\nTras esta declaraci\xF3n, se ejecuta el m\xE9todo `setState` heredado de R.Component al cual se env\xEDa una funci\xF3n (y siempre deber\xE1 ser as\xED) en cuyo seno se enlaza a un objeto que corresponde a `state` del cual se modificar\xE1 el estado previo `(prevState)` de `tasks` increment\xE1ndolo en uno.\nSeguidamente, se inserta en el Componente HTML `SimpleTodo` que se cre\xF3 en la l\xEDnea 10., el Elemento HTML creado en la variable `newtask` utilizando su m\xE9todo `getTag()`.\nEn la \xFAltima l\xEDnea de c\xF3digo ejecutable se hace uso de otro m\xE9todo del M\xF3dulo de Rocfor.js ___r___ (Resources), `focusAndClean`, que hace eso precisamente, limpia el contenido del Elemento HTML `input txt` y asigna el foco del cursor en este elemento; observe que el par\xE1metro que recibe es lo que devuelve el m\xE9todo `getChild()` del Objeto `todo` creado en la l\xEDnea 10., al cual se le pasa una cadena que corresponde al identificador del Elemento HTML `input text de id=\"todo\"`.\n___\xA1Rocfor.js!___\n?  \n?  \n\nid(tuto-aside)class(Tuto-aside.one.one-g-20.m-top-5.rel.d-none.M-d-block)? \nid(tuto-menu)class(thm-listnone.fix)* [Inicio e instalaci\xF3n](#init \"Ver c\xF3mo iniciar con Rocfor.js\") \n* [M\xF3dulo Tag](#tag \"Crear un Elemento HTML con Tag\")\n* [M\xF3dulo Block](#block \"Crear un Bloque de elementos HTML con Block\")\n* [Component](#comp \"Crear un Componente Reactivo con Component\")\n* [Construir una lista de tareas](#todo \"Crear un Todo list\")  \n?  \n\n?  ")
  };
};
"use strict";
...
function (_R$Component) {
  ...
  function Main(props) {
    ...
    _this.contents = new Contents();
    _this.mainheader = _this.r.$('home');
    _this.header = _this.r.$('header');
    _this.home = _this.Component();
    _this.tuto = _this.Component();
    _this.samples = _this.Component();
    _this.footer = _this.Component();
    _this.docs = _this.Component();
    _this.fhf = _this.Component();
...
/*#__PURE__*/
function (_R) {
  _inherits(SPAIndex, _R);

  function SPAIndex(props) {
    var _this;

    _classCallCheck(this, SPAIndex);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(SPAIndex).call(this, props));
    _this.main = new Main(_this.props.containers);
    _this.main = _this.main.buildMain();
    _this.spa = _this.SPA();
    _this.scom = new Simplecomponent({
      container: 'simplecomp'
    });
    _this.todo = new SimpleTodo({
      container: 'simpletodo'
    });
    _this.countdown = new Countdown({
      container: 'simplecountdown'
    });
    _this.simtodo = new SimpleToDo();
    _this.createControllers = _this.createControllers.bind(_assertThisInitialized(_this));

    _this.createRoutes();

    _this.createControllers(); // console.log(`${this.main.docs.classlist.join(' ')}\n\n${this.main.home.classlist.join(' ')}\n\n${this.main.tuto.classlist.join(' ')}\n\n${this.main.samp.classlist.join(' ')}\n\n${this.main.fhf.classlist.join(' ')}`)


    return _this;
  }

  _createClass(SPAIndex, [{
    key: "createRoutes",
      ...
      this.spa.setRoutes({
        '/': {
          ...
          'callback': function callback() {
            // c('This is my Home!')
            _this2.spa.playRoute().home(_this2);

            _this2.scom.scom.render();

            _this2.todo.todo.render();

            _this2.countdown.counter.render();

            _this2.countdown.runCounter(); // this.scom = new Simplecomponent({container:'simplecomp'})
        ...
        'docs': {
          ...
          'controller': 'docs',
          'callback': function callback() {
            _this2.spa.playRoute().docs(_this2);
          }
        },
        'tutorial': {
          ...
          'callback': function callback() {
            ...
            _this2.spa.playRoute().tuto(_this2);

            _this2.simtodo.todo.render('mysimpletodo');
          }
        },
        'samples': {
